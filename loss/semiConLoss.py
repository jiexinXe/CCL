"""
Author: ZiHao Zhou
Date: May 07, 2024
"""

import torch
import torch.nn as nn
import torch.nn.functional as F


def unlabelProbability(label_feat, unlabel_feat, label, device, la=0):
    alpha = 0.2
    simuu = torch.matmul(unlabel_feat, unlabel_feat.t())
    simuu = F.softmax(simuu, dim=1)
    mat = torch.inverse(torch.eye(simuu.shape[0]).to(device) - alpha * simuu)
    simul = torch.exp(torch.matmul(unlabel_feat, label_feat.t()) / 0.1)

    py_sum = torch.sum(label, dim=0)
    py_sum = py_sum.repeat(simul.size(0), 1)
    logits = torch.log(torch.matmul(simul, label) / py_sum)
    logits = logits + la
    pl = (1 - alpha) * F.softmax(logits, dim=1)
    return torch.matmul(mat, pl)


def crossLoss(a, b):
    return -torch.mean(torch.sum(a * torch.log(b), dim=1), dim=0)


def sharp(a, T):
    a = a ** T
    a_sum = torch.sum(a, dim=1, keepdim=True)
    a = a / a_sum
    return a.detach()


class SemiConLoss(nn.Module):
    def __init__(self, labeled_bs, unlabeled_bs, num_class, args, temperature=0.1):
        super(SemiConLoss, self).__init__()
        self.temperature = temperature
        self.labeled_bs = labeled_bs
        self.unlabeled_bs = unlabeled_bs
        self.num_class = num_class
        self.args = args
        self.py_unlabeled = args.py_uni.to(args.device).clamp(min=1e-12)  # 避免py_unlabeled为0
        self.py_Num = torch.zeros(num_class, device=args.device)

    def forward(self, feat, label, la=0, epsilon=1e-12):
        label = label.float()
        # 特征归一化，稳定后续计算
        feat = F.normalize(feat, dim=1)

        # 切片时添加边界检查（避免索引越界导致的异常值）
        total_feat = feat.shape[0]
        split_idx = self.num_class + 2 * self.labeled_bs
        if split_idx > total_feat:
            split_idx = total_feat  # 防止索引越界
        label_feat = feat[:split_idx, :]
        unlabel_feat = feat[split_idx:, :]

        # 确保unlabel_feat可被3等分（避免chunk失败）
        if unlabel_feat.shape[0] % 3 != 0:
            pad = 3 - (unlabel_feat.shape[0] % 3)
            unlabel_feat = torch.cat([unlabel_feat, torch.zeros(pad, unlabel_feat.shape[1], device=self.args.device)],
                                     dim=0)
        anchor_feat, unlabel_feat1, unlabel_feat2 = unlabel_feat.chunk(3)

        with torch.no_grad():
            up_aim = unlabelProbability(label_feat, anchor_feat, label, self.args.device, la)
            self.py_Num += torch.sum(up_aim, dim=0).clamp(min=epsilon)  # 避免累积异常值
            up_aim = sharp(up_aim, 4.0)

        up1 = unlabelProbability(label_feat, unlabel_feat1, label, self.args.device, la)
        up2 = unlabelProbability(label_feat, unlabel_feat2, label, self.args.device, la)
        up_u = (up1 + up2) / 2

        # 检查概率分布是否有效（避免无效值参与计算）
        up_aim = up_aim.clamp(min=epsilon)
        up1 = up1.clamp(min=epsilon)
        up2 = up2.clamp(min=epsilon)
        up_u = up_u.clamp(min=epsilon)

        loss = (crossLoss(up_aim, up1) + crossLoss(up_aim, up2) + crossLoss(up_u, up_u)) / 3
        self.py_Num += torch.sum(up_u.detach(), dim=0).clamp(min=epsilon)
        return loss


class softConLoss(nn.Module):
    def __init__(self, labeled_bs, unlabeled_bs, num_class, args, temperature=0.1):
        super(softConLoss, self).__init__()
        self.temperature = temperature
        self.labeled_bs = labeled_bs
        self.unlabeled_bs = unlabeled_bs
        self.num_class = num_class
        self.args = args
        self.criterion = nn.CrossEntropyLoss(reduction='mean')

    def forward(self, label, feat, device, la=0, epsilon=1e-12):
        label = label.float()
        # 特征归一化
        feat = F.normalize(feat, dim=1)

        # 控制exp输入范围，避免数值溢出
        sim = torch.exp(torch.matmul(feat, feat.t()) / self.temperature) + epsilon

        py_sum = torch.sum(label, dim=0, keepdim=True)
        py_sum = py_sum.repeat(sim.size(0), 1) - label
        py_sum = py_sum.clamp(min=epsilon)  # 确保分母不为0

        matrix = torch.ones((label.shape[0], label.shape[0]), device=device)
        matrix = matrix.fill_diagonal_(0)
        sim = sim * matrix

        # 避免log(0)和除法异常
        logits = torch.log((torch.matmul(sim, label) / py_sum) + epsilon)

        # 切片边界检查
        if self.num_class > logits.shape[0]:
            self.num_class = logits.shape[0]  # 防止索引越界
        logits = logits[self.num_class:, :]
        logits = logits + la
        label = label[self.num_class:, :]

        # 确保label有效（避免无效标签导致的损失异常）
        label = label.clamp(min=epsilon)
        loss = self.criterion(logits, label)
        return loss
