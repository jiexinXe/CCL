...
class RunRecorder:
    ...
    # --- CSV-only saving helpers ---
    def _append_vector_row(self, path: Path, epoch: int, vec: torch.Tensor, name: str):
        """Append a 1×K row with header epoch,class_0,...,class_{K-1}"""
        if not self.enable: return
        arr = vec.detach().float().cpu().numpy().reshape(1, -1)
    ...
    def save_vector(self, name:str, epoch:int, vec:torch.Tensor):
        # write/append to vectors/{name}.csv
        path = self.out / "vectors" / f"{name}.csv"
        self._append_vector_row(path, epoch, vec, name)

    def save_matrix(self, name:str, epoch:int, mat:torch.Tensor):
        """Save K×K matrix to matrices/{name}_epochXXX.csv with row/col labels"""
        if not self.enable: return
        arr = mat.detach().cpu().numpy()
        df = pd.DataFrame(arr,
                          columns=[f"pred_{j}" for j in range(arr.shape[1])],
                          index=[f"true_{i}" for i in range(arr.shape[0])])
        path = self.out / "matrices" / f"{name}_epoch{epoch:03d}.csv"
        df.to_csv(path)

    def save_energy_csv(self, name:str, epoch:int, payload:dict):
        """Append diagnostics row to diagnostics/{name}.csv"""
        if not self.enable: return
        path = self.out / "diagnostics" / f"{name}.csv"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.local_rank in [-1, 0]:
            ...
            if args.local_rank in [-1, 0]:
                ...
                # save vectors/matrix for this epoch (CSV)
                ...
                # diagnostics (energy) CSV
                args.recorder.save_energy_csv("energy", epoch, {
